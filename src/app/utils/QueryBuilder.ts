...
> {
    ...
    include(relation : TInclude) : this {
        ...
        // if fields method is used, include will be ignored to prevent conflict between select and include
    ...
    dynamicInclude(
        ...
        if(includeParam && typeof includeParam === 'string') {
            const requestedRelation = includeParam.split(",").map(relation => relation.trim());
    ...
    private deepMerge(target : Record<string, unknown>, source : Record<string, unknown>) : Record<string, unknown> {
        ...
        for(const key in source){
            if(source[key] && typeof source[key] === 'object' && !Array.isArray(source[key])){
                if(result[key] && typeof result[key] === 'object' && !Array.isArray(source[key])){
